-- EGC usage board: carry how long each publisher has actually had a seat.
--
-- "0 posts in 30 days" reads as a 30-day track record. For most of this book
-- there is no such record to have: Justin Mongroo and Brennen Smith got seats
-- 9 days ago, Thrad's two 4 and 5 days ago, Buzzlead's two 15 days ago. Six of
-- ten publishers have been in the product for less than the window we were
-- reporting against, so the tab was quietly holding brand-new accounts to a
-- month of silence they never had the chance to fill.
--
-- users.created_at is the seat, and it matches auth.users.created_at on every
-- row. It is deliberately NOT user_companies.created_at: Runpod as a company
-- goes back to February, but nobody there had a publisher seat until 20 August,
-- and the seat is what the per-person numbers are measured against.
