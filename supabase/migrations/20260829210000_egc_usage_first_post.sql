-- EGC usage board: carry each publisher's FIRST post, for time to value.
--
-- Time to value is kickoff to first post going live. Kickoff lives in HubSpot;
-- the first post has to come from Lineage, because HubSpot's first_post_date is
-- filled in by hand and is null on four of the five EGC accounts — Runpod is the
-- only one set, and it records a Full Service post from March, not the EGC seat's
-- first post in August.
--
-- Both first_post and last_post are deliberately unwindowed. Time to value is
-- measured from kickoff, which can sit outside whatever 30- or 90-day window the
-- rest of the tab happens to report on: Othello kicked off on 1 June and first
-- published on 17 July, and a 30-day window would show neither.
